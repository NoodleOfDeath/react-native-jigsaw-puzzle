...
export const JigsawPuzzle: React.FC<JigsawPuzzleProps> = ({
  ...
  const [tiles, setTiles] = React.useState<Tile[] | undefined>()
  const [imageSize, setImageSize] = React.useState<{ width: number, height: number }>()
  const [rootSize, setRootSize] = React.useState<{ width: number, height: number }>()
  const [calculatedHeight, setCalculatedHeight] = React.useState<number>()
  const rootElement = React.useRef<HTMLElement>()
  const resizeObserver = React.useRef<ResizeObserver>()
  const draggingTile = React.useRef<{ tile: Tile, elem: HTMLElement, mouseOffsetX: number, mouseOffsetY: number } | undefined>()
  ...
  const onRootElementResized = React.useCallback((args: ResizeObserverEntry[]) => {
  ...
  const onRootElementRendered = React.useCallback((element: HTMLElement | null) => {
    if (element) {
  ...
  const onTileMouseDown = React.useCallback((tile: Tile, event: React.MouseEvent<HTMLDivElement> | React.TouchEvent<HTMLDivElement>) => {
    if (!tile.solved) {
  ...
  const onRootMouseMove = React.useCallback((event: React.MouseEvent<HTMLDivElement> | React.TouchEvent<HTMLDivElement>) => {
  ...
  const onRootMouseUp = React.useCallback((event: React.TouchEvent | React.MouseEvent) => {
    if (draggingTile.current) {
  ...
  return <View ref={onRootElementRendered}
